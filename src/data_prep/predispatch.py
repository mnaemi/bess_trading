...
def find_remote_predispatch_zips(target_date: str, base_url: str = "https://nemweb.com.au/Reports/ARCHIVE/Predispatch_Reports/") -> List[str]:
    """
    Find ZIP file URLs on AEMO NEMWeb whose date range includes the target_date.
    :param target_date: 'YYYY-MM-DD'
    :param base_url: URL to the archive
    :return: List of matching full URLs
    """
    html = requests.get(base_url).text
    zip_files = sorted(set(re.findall(r'PUBLIC_PREDISPATCH_\d{8}_\d{8}\.zip', html)))
...
def unzip_file(zip_path: str, extract_to: str = "predispatch_extract") -> List[str]:
    """
...
def find_target_date_zipfiles(folder_path: str, target_date: str, extract_to: str = "predispatch") -> List[str]:
    """
...
#%%
    ...
    urls = find_remote_predispatch_zips(target_date)
    ...
    # Step 3: Unzip it
    top_level_files = unzip_file(downloaded_zip, extract_to="predispatch_extract")

    # Step 4: Find and extract nested ZIPs for the target day
    matched_csvs = find_target_date_zipfiles("predispatch_extract", target_date, extract_to="predispatch")
    df = load_vic1_rows_from_csvs('predispatch')
    df.to_csv(f"vic1_predispatch_{target_date}.csv", index=False)
